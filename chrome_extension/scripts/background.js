...
function checkUrlWithRetry(url, tabId, retries) {
    ...
    .then(data => {
        ...
        // Map backend risk_level to extension result
        let riskLevel = (data.risk_level || "").toLowerCase();
        let result;
        if (riskLevel === "high") result = "malicious";
        else if (riskLevel === "medium") result = "suspicious";
        else if (riskLevel === "low") result = "safe";
        else result = "unknown";
        let reasons = Array.isArray(data.reasons) ? data.reasons : [];
        storeAndHandleResult(url, tabId, result, reasons);
    })
    .catch(err => {
...
function storeAndHandleResult(url, tabId, result, reasons) {
    lastCheckedUrls[url] = { result, reasons, timestamp: Date.now() };
    ...
    handleResult(url, tabId, result, reasons);
}
function handleResult(url, tabId, result, reasons) {
    console.log(`Handling URL Result: ${url}, Tab=${tabId}, Result=${result}`);

    let reasonsText = (Array.isArray(reasons) && reasons.length)
        ? "\n• " + reasons.join("\n• ")
        : "";
    ...
    if (result === "malicious") {
        ...
        chrome.notifications.create({
            ...
            message: "This website has been flagged as malicious." + reasonsText + "\nClick to dismiss future warnings.",
            priority: 2,
        ...
        chrome.notifications.create({
            ...
            message: "This website has been flagged as suspicious." + reasonsText,
            priority: 1
        });
    }
}
